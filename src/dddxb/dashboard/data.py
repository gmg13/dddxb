...
and the ``--history`` ingest). Loaders return ``None`` when the expected file is
absent so the UI can show the exact command to run instead of a traceback.
...
def available_windows() -> list[int]:
    """Windows (months) for which a ranking parquet exists, longest first."""
    found = []
    for window in (24, 12, 6, 3, 1):
        if _ranking_path(window).exists():
            found.append(window)
    return found
...
def load_ranking(window: int) -> pl.DataFrame | None:
    path = _ranking_path(window)
    return pl.read_parquet(path) if path.exists() else None


@st.cache_data(show_spinner=False)
def load_cohorts(window: int) -> pl.DataFrame | None:
    path = _cohort_path(window)
    return pl.read_parquet(path) if path.exists() else None


@st.cache_data(show_spinner=False)
def load_clean_sales(window: int) -> pl.DataFrame | None:
    path = Path(PROCESSED_DIR) / f"for_sale_clean_last{window}m.parquet"
    return pl.read_parquet(path) if path.exists() else None


@st.cache_data(show_spinner=False)
def load_clean_rents(window: int) -> pl.DataFrame | None:
    path = Path(PROCESSED_DIR) / f"for_rent_clean_last{window}m.parquet"
    return pl.read_parquet(path) if path.exists() else None
...
def history_available(years: int = HISTORY_YEARS) -> bool:
    return _history_path(years).exists()
